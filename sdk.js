...
function postMessageToApp(eventType, data) {
  if (!centricientHost) {
    throw new Error('You need to call `init` before posting messages to Quiq');
  }
 ...
 */
Quiq.prepareMessage = function(message, method) {
  checkConversationId();
 ...
 */
Quiq.sendOnClose = function(message) {
  checkConversationId();
 ...
 */
Quiq.updateContactDisplayName = function(contact) {
  checkConversationId();
 ...
 */
Quiq.getConversation = function() {
  // TODO: Should we just add `conversation` to Quiq instead of making a getter?
  return conversation;
};
...
function asyncRequest(requestType, data) {
  return new Promise(function(resolve, reject) {
    Quiq.on(requestType + '.success' + requestCounter, function(data) {
      resolve(data.data);
    });

    Quiq.on(requestType + '.error' + requestCounter, function(error) {
      reject(error);
    });
 ...
 * @returns {Promise}
 */
Quiq.getExtensionData = function() {
 ...
 */
Quiq.setExtensionData = function(data) {
